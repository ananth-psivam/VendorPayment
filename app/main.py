...
# --- Optional parsers (used if installed) ---
try:
    import pdfplumber
except Exception:
    pdfplumber = None
...
TABLE_NAME = "invoices"  # change if needed

# Your table columns (as you showed):
...
def is_payment_inquiry(text: str) -> bool:
    ...
    # If we see "invoice" + any payment intent, it’s very likely a payment inquiry
...
def storage_list(sb: Client, bucket: str, prefix: str = "", include_one_subfolder: bool = True) -> List[str]:
    """
    List files under bucket/prefix, optionally include one-level subfolder contents.
...
def draft_email(vendor_name: str, vendor_email: Optional[str], inv_no: str, row: Optional[Dict]) -> Tuple[str, str]:
    """
    Returns (subject, body) – body is plain text.
    Uses Status and Comments from your schema.
    """
...
def main():
    ...
    st.caption("Reads PDF/HTML from Supabase Storage, detects payment inquiries, extracts invoice numbers, "
               "checks Supabase status, and drafts a reply email.")
    ...
    if st.button("Process selected files"):
        for idx, path in enumerate(selected, start=1):
            ...
            if not invoice_ids:
                # Draft generic request for more info
            ...
            # Look up all invoice IDs by Supplier_Invoice_No
